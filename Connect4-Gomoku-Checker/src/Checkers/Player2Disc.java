...
public class Player2Disc extends CheckerDisc {

    public Player2Disc(int row, int column, Color color){
        super(row, column, color);
    }

    public boolean isForwardDiagonal(Position position){
        return  (this.position.getColumn() - position.getColumn() == -1 && //forward left
                this.position.getRow() - position.getRow() == 1) ||
                (this.position.getColumn() - position.getColumn() == 1 && //forward right
                this.position.getRow() - position.getRow() == 1);
    }

    public boolean isBackwardDiagonal(Position position){
        return  (this.position.getColumn() - position.getColumn() == -1 && //backward left
                this.position.getRow() - position.getRow() == -1) ||
                (this.position.getColumn() - position.getColumn() == 1 && //backward right
                this.position.getRow() - position.getRow() == -1);
    }
